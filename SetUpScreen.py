...
difficultyUpPos = (arrowUpxPos, 300 + yAlignment)
difficultyDownPos = (arrowDownxPos, 300 + yAlignment)
timeUpPos = (arrowUpxPos, 400 + yAlignment)
timeDownPos = (arrowDownxPos, 400 + yAlignment)

arrows = {timeDownPos, timeUpPos, difficultyDownPos, difficultyUpPos, yDownPos, yUpPos, xDownPos, xUpPos}
...
class SetUp:
        ...
        for event in pygame.event.get():
            if event.type == QUIT:
                ...
                for arrow in arrows:
                    if self.CheckTounching(pos, arrow, self.upArrow.get_size()):
                        if arrow == xUpPos:
                            ...
                            if self.ySize > 2:
                                self.ySize -= 1
                        elif arrow == difficultyUpPos:
                            if self.difficulty < 60:
                                self.difficulty += 1
                        elif arrow == difficultyDownPos:
                ...
                if self.CheckTounching(pos, goButtonPos, self.goButton.get_size()):
                    return (self.xSize, self.ySize, 61 - self.difficulty, self.time)
    ...
    def Run(self):
        while(1):
            ...
            zString = 'Difficulty (1 - 60): ' + str(self.difficulty)
